...
class TestSparseEmbedID(unittest.TestCase):

    def setUp(self):
        self.link = links.SparseEmbedID(3, 2, ignore_label=self.ignore_label)
        self.link.ignore_label
        self.link.zerograds()
        print(getattr(self.link, '0').data)

        self.W = numpy.stack(
            [getattr(self.link, str(i)).data for i in range(3)])
    ...
    def check_backward(self, x_data, y_grad):
        gradient_check.check_backward(
            self.link, x_data, y_grad, tuple(self.link.params()))
...
class TestSparseEmbedIDValueCheck(unittest.TestCase):

    def setUp(self):
        self.link = links.SparseEmbedID(2, 2, ignore_label=self.ignore_label)
