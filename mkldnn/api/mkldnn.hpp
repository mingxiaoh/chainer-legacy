...
struct concat : public primitive {
    ...
    concat(const primitive_desc &concat_pd,
        ...
        std::vector<primitive::at> at_inputs;
        for (size_t i = 0; i < inputs.size(); i++)
            //at_inputs.push_back(inputs[i]);
            at_inputs.push_back(primitive::at(inputs[i], i));
        concat(concat_pd, at_inputs, output);
    }
#else
    concat(const primitive_desc &concat_pd,
            std::vector<primitive> &inputs, const memory &output) {
        ...
        std::vector<c_api::mkldnn_primitive_at_t> p_inputs;
        for (size_t i = 0; i < inputs.size(); i++) {
            primitive::at at(inputs[i]);
            p_inputs.push_back(at.data);
        }
        c_api::const_mkldnn_primitive_t outputs[] = { output.get() };
