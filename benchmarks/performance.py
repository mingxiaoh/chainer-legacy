...
def test_performance(modelName, data_size, batch_size, insize, epoch, log):
    ...
    for i in range(epoch):
        ...
        data = np.random.uniform(-1, 1, (batch_size, 3, insize, insize)).astype(np.float32)
        x = np.asarray(data)
    ...
    __log__(log, "Average total (ms): {0}\n".format((total_forward + total_backward) / count))
    ...
    result = {
        ...
        "Total": round(data_size * 1000 * count / (total_forward + total_backward), 3)
    }
...
if __name__ == "__main__":
    ...
    This script is for the performance test. You can use it to perform test by specifying -a, -b and -i.
    """)
    ...
    parser.add_argument("--log", "-l", default=None, help="Log file path. If empty, the script will use stderr.")
    ...
    if args.arch in archs:
        ...
        __log__(log, "arch: {0}, batchsize: {1}, datasize: {2}, epoch: {3}, insize: {4}\n".format(args.arch,
                                                                                                  args.batchsize,
                                                                                                  args.datasize,
                                                                                                  args.epoch,
                                                                                                  args.insize))
        test_result = test_performance(args.arch, args.datasize, args.batchsize, args.insize, args.epoch, log)
