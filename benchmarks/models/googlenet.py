...
class GoogLeNet(chainer.Chain):
    ...
    def forward(self, x):
        ...
        if chainer.config.train:
            out1 = F.average_pooling_2d(h, 5*self.insize//224, stride=3*self.insize//224)
        ...
        if chainer.config.train:
            out2 = F.average_pooling_2d(h, 5*self.insize//224, stride=3*self.insize//224)
        ...
        h = F.dropout(F.average_pooling_2d(h, 7*self.insize//224, stride=1*self.insize//224), 0.4)
